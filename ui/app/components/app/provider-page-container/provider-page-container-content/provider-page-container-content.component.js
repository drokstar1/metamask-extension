...
export default class ProviderPageContainerContent extends PureComponent {
  ...
  renderConnectVisual = (title) => {
    const { origin, selectedIdentity, siteImage } = this.props
  ...
  render () {
    const { siteTitle, hostname } = this.props
    const { t } = this.context

    const title = siteTitle || hostname

    return (
      <div className="provider-approval-container__content">
        <section>
          <h2>{t('connectRequest')}</h2>
          {this.renderConnectVisual(title)}
